...
class SimpleAiEntity(eclipse.Entity):
    ...
    def update(self, dt:float):
        ...
        if self.target is not None:
            ...
            if abs(self.target.position.x - self.position.x) < 10:
                move_x = 0

            target_velocity = 120*move_x
            if keys_pressed[K_a]:
                e.slipperiness_x = 1
                e.velocity.x += min(0, (-target_velocity-e.velocity.x))*dt*acceleration
            elif keys_pressed[K_d]:
                e.slipperiness_x = 1
                e.velocity.x += max(0, (target_velocity-e.velocity.x))*dt*acceleration
